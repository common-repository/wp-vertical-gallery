...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ vrt_upload_dir+"/"+file.name + '&task=vrt_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	vrt_refresh_images_table(vrt_current_album_id);
...
function vrt_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=vrt_get_albums_table', function(res)
	{
		vrt_set_data_tables();
		if (res.status == 'ok') 
		{
...
function vrt_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=vrt_get_albums_images_table&album_id='+album_id, function(res)
	{
		vrt_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'vrt_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'vrt_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'vrt_reorder_image' )
		{
			vrt_refresh_images_table(album_id);
		}
		if( task == 'vrt_reorder_album' )
			vrt_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function vrt_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', vrt_submit_bulk);
...
function vrt_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'vrt_bulk_delete_albums' || task == 'vrt_bulk_enable_albums' || task == 'vrt_bulk_disable_albums')
	{
	...
	else if( task == 'vrt_bulk_delete_images' || task == 'vrt_bulk_enable_images' || task == 'vrt_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = vrt_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: vrt_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: vrt_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: vrt_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: vrt_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : vrt_url+"/js/swfupload/js/swfupload.swf",
				upload_url: vrt_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":vrt_upload_dir, "secw":vrt_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: vrt_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.vrt_delete_album').live('click', vrt_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', vrt_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', vrt_disable);
